CREATE OR REPLACE TABLE gold.monthly_loss_trends AS
WITH base as (
    SELECT 
      DATE_TRUNC('month', LossDate) AS loss_month,
      ...
      SUM(CASE WHEN Loss = 1 THEN OffsetValue ELSE 0 END) total_loss_offset_value,
    ...
    GROUP BY DATE_TRUNC('month', LossDate)
)
SELECT
  loss_month,
  total_customers,
  ...
  'monthly_loss_trends' AS table_version,
